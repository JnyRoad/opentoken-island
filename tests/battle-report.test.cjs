...
// 因为是纯函数，这里用最朴素的 node assert 直接驱动，不引入测试框架，
// 与项目既有的 windows_support_contract.test.cjs 风格保持一致。
//
// 覆盖维度：
...
const { buildBattleReport, NEAR_THRESHOLD } = require("../battle-report");
...
scenario("leadOverNext 占比低命中 defense，并点名追兵", () => {
  const report = buildBattleReport({
...
scenario("gapToPrevious 占比低命中 soon，并点名上一名", () => {
  const report = buildBattleReport({
    own: { rank: 5, score: 100_000_000, name: "我" },
...
scenario("rankDelta>0 且距离都远时命中 climb，措辞为较上次", () => {
  const report = buildBattleReport({
    own: { rank: 5, score: 100_000_000, name: "我" },
    previous: { name: "张三", score: 130_000_000 },
    next: { name: "王五", score: 50_000_000 },
...
scenario("rank=1 且 rankDelta=0 命中 default（守榜）", () => {
  const report = buildBattleReport({
...
scenario("同时满足 soon 与 climb 时优先 soon", () => {
  const report = buildBattleReport({
    own: { rank: 5, score: 100_000_000, name: "我" },
    previous: { name: "张三", score: 103_200_000 },
    next: { name: "王五", score: 50_000_000 },
...
scenario("同时满足 defense 与 soon 时优先 defense", () => {
  const report = buildBattleReport({
    own: { rank: 5, score: 100_000_000, name: "我" },
    previous: { name: "张三", score: 103_200_000 },
    next: { name: "王五", score: 98_000_000 },
...
scenario("缺少 own 时返回 default 且 metric 为占位", () => {
...
scenario("own.score=0 时不触发 soon/defense，落到 default", () => {
  const report = buildBattleReport({
...
// 以下三类靠 && 短路保护的分支，补测试钉死回归。

// 榜尾：next 为 null 时，即便领先差很薄也不该触发 defense（无身后追兵）。
scenario("next 为 null（榜尾）时不触发 defense", () => {
  const report = buildBattleReport({
...
scenario("previous 为 null 时不触发 soon", () => {
  const report = buildBattleReport({
    own: { rank: 5, score: 100_000_000, name: "我" },
    previous: null,
    next: { name: "王五", score: 50_000_000 },
...
scenario("own.score=0 仍能触发 overtaken", () => {
  const report = buildBattleReport({
...
if (failed > 0) {
  console.error(`\n${failed}/${cases.length} battle-report 用例失败`);
  process.exit(1);
}
console.log(`\nbattle-report: ${cases.length}/${cases.length} 通过`);
