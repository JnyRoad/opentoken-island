...
//
// 命名说明：本模块产出的是「灵动岛上要弹/要展示的那一条战报」（buildBattleReport），
// 与同目录的 battle-report.js（buildGame：等级/任务/徽章面板）职责不同——前者是一条推送，
// 后者是整块游戏面板，故拆成两个模块（SRP）。
//
// 设计取舍（第一性原理）：
 ...
 *
 * @param {object} board 排行榜快照，对应 lib/summary.js 的 leaderboard
...
function buildBattleReport(board) {
  ...
  const estimated = Boolean(board.estimated || own.estimated);

  if (estimated) {
    return {
      type: "default",
      icon: "award",
      title: rank ? `预计 ${metric}` : "等待排名",
      copy: "等待榜单确认",
      metric,
    };
  }
