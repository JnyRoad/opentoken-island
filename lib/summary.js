...
function computeLeaderboard(entries, summary, previousRank, userId, options = {}) {
  ...
  if (own) {
    ...
    previous = ownRank > 1
      ? entries.find((entry) => Number(entry.rank || 0) === ownRank - 1) || null
      : null;
    next = entries.find((entry) => Number(entry.rank || 0) === ownRank + 1) || null;
  } else {
...
function buildSummary({ lastUpload, leaderboard }) {
  ...
  const gap = board?.gapToPrevious === null || board?.gapToPrevious === undefined
    ? null
    : Number(board.gapToPrevious || 0);
  const lead = board?.leadOverNext === null || board?.leadOverNext === undefined
    ? null
    : Number(board.leadOverNext || 0);
