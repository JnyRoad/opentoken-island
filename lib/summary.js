...
function leaderboardScore(summary = {}) {
  return Number(summary.normalized || 0) > 0
    ? Number(summary.normalized || 0)
    : Number(summary.total || 0);
}

function leaderboardTools(summary = {}) {
  return Number(summary.normalized || 0) > 0
    ? (summary.normalizedByTool || {})
    : (summary.byTool || {});
}
...
function findOwnEntry(entries, summary, userId) {
  ...
  if (userId) {
    return matches.find((entry) => String(entry.userId) === String(userId)) || matches[0];
  }
  return matches[0];
}
...
function computeLeaderboard(entries, summary, previousRank, userId, options = {}) {
  ...
  if (own) {
    ...
    previous = ownRank > 1
      ? entries.find((entry) => Number(entry.rank || 0) === ownRank - 1) || entries[index - 1] || null
      : null;
    next = entries.find((entry) => Number(entry.rank || 0) === ownRank + 1) || entries[index + 1] || null;
  } else {
