...
function buildGame({ total, rank, rankEstimated = false, rankDelta, byTool, previous, next, gap, lead, accepted = 0 }) {
  ...
  const rankQuest = king
    ...
    : {
        icon: "trending-up",
        title: rankEstimated ? `榜单同步中：预计 #${rank}` : "排名冲刺：超过上一名",
        detail: rankEstimated
          ? (next ? `本地估算领先 ${next.name} ${formatCount(lead)}` : "等待榜单收录当前上传")
    ...
    badges: [
      {
        icon: "crown",
        title: "King Mode",
        detail: king ? "今日总榜 #1" : rank ? `${rankEstimated ? "预计 " : "当前 "}#${rank}` : "等待排名",
        unlocked: king,
        featured: king,
      },
      {
      ...
      },
      {
        icon: "trending-up",
        title: "Rank Climber",
        detail: rankEstimated ? "等待榜单确认" : rankDelta > 0 ? `上升 ${rankDelta} 名` : king ? "守住第 1" : "等待突破",
